...
class Player:
    ...
    def move(self, battle_data: dict) -> dict:
        ...
        for player_index in battle_data.get("players"):
            if player_id == player_index.get("id"):
                ...
                self.control_ball = True if self.player_dict[
                                                "controlBall"] == 1 else False
            if player_index.get("teamId") == TEMAID and player_index.get(
                    "id") not in TEAMMATE:
            ...
            if player_index.get("teamId") != TEMAID and player_index.get(
                    "controlBall") == 1:
                self.ennermy_controll_ball = Point(player_index.get("col"),
                                                   player_index.get("row"),
                                                   player_index.get("state"))
        self.players = battle_data.get("players")
        graph = Graph(self.map_list, self.player_dict)
        self.round = battle_data.get("round")
        print("playerId :{} row:{},col:{}".format(self.player_id,
                                                  self.player_dict["row"],
                                                  self.player_dict["col"]))
        # 1. 判断是否运用路障 OR 炸药包
        ...
        if self.control_ball:
            decision = self.captain_decision(graph)  # 持球人策略
        else:
            # decision = self.captain_decision()
            decision = self.help_decision()  # 辅助策略
        return decision
        ...
        for player_index in battle_data.get("players"):
            if str(self.team_id) != player_index.get("teamId"):
                ...
                if self.heuristic(enemy,goal) <= 2:
                    index = self.final_defense(enemy, goal, index, x, y)
                elif self.heuristic(enemy, goal) in range(3,5):
                # 当距离为4时；基于其最短路径进行防御
                # TODO: extract methods
                    ...
                    path = dict()
                    path[enemy] = None
                    enemy_shorest_path = [graph.goal]
                    # 进行BFS搜索，queue不为空
                    while queue:
                        current_point: Point = queue.get()
                        # 若当前的点就是终点，则搜索结束
                        if current_point == graph.goal:
                            while path[current_point] != None:
                                enemy_shorest_path.append(path[current_point])
                                current_point = path[current_point]
                            break

                        for next in graph.neighbors(current_point):
                            if next not in path:
                                path[next] = current_point
                                queue.put(next)
                    enemy_shorest_path = enemy_shorest_path.reverse()
                    print("Enermy Path: ", enemy_shorest_path)
                else:
                    pass
    ...
    def final_defense(self, enemy, goal, index, x, y):
        if self.heuristic(enemy, goal) == 2:
    ...
    def captain_decision(self, graph):
        ...
        work_path = self.path_finding(graph, start)
        ...
        if length == 1:
            ...
            if type(decision) == tuple:
                # todo 如果想最大化分数 ，可以调整阈值 step, 大于这个step的时候就不投球呢，最大也就是3. last_tool_num 为最后剩余的道具数量
                # 大于阈值不投球， 小于阈值立刻投球
                step = 2
                last_tool_num = 2
                if self.heuristic(self.ennermy_controll_ball,
                                  graph.goal) > step and len(
                    graph.all_tools) > last_tool_num and self.round < 290:
                    action["actionType"] = "N"
                    return action
    ...
    def path_finding(self, graph, start) -> list:
    ...
    def help_decision(self):
        ...
        if not ready_send and controll_ball and self.player_dict[
            "blockNum"] != 0:
            print("attack controll ball")
            action["actionType"] = "B"
            action["actionDesc"] = str(controll_ball.y) + "," + str(
                controll_ball.x)
            return action
    ...
    def attack_controll_ball(self, graph):
        ...
        '''
        for player in self.players:
            if player.get("teamId") != TEMAID and player.get(
                    "controlBall") == 1:
                point = Point(player["col"], player["row"], player["id"])
                goal_path = self.path_finding(graph, point)
                if len(goal_path) > 1:
                    state = goal_path[1].state
                    goal_path[1].state = "H"
                    goal_path1 = self.path_finding(graph, point)
                    goal_path[1].state = state
                    if len(goal_path1) - len(goal_path) > 4 and goal_path[
                        1].state == "O":
                        print("emermy position, row:{},col:{}".format(point.y,
                                                                      point.x))
                        print("emermy path:{}", goal_path)
                        block_point = goal_path[
                            1]  # 0 为持球人所在的位置， 1为该持球人下个人要走的位置
                        return block_point
                return None
    ...
    def find_goal_path(self, graph, start, ready_send=False):
        ...
        if len(all_goal_path) > 1:
            path1, cost1 = self.path_finding_teammate(graph, start,
                                                      all_goal_path[0])
            path2, cost2 = self.path_finding_teammate(graph, start,
                                                      all_goal_path[1])
            if cost1 > cost2:
                return path2
            if cost1 < cost2:
